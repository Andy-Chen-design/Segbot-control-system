...
uint16_t RCinp1 = 0;
uint16_t RCinp2 = 0;
uint16_t servo_increment = 0;
float ADC1out = 0.0;
float ADC2out = 0.0;
uint16_t second_actual = 30;
uint16_t minute_actual = 51;
uint16_t hour_actual = 16;
uint16_t day_actual = 3;
uint16_t date_actual = 31;
uint16_t month_actual = 10;
uint16_t year_actual = 23;
...
int16_t WriteDAN777RCServo(uint16_t RC1, uint16_t RC2) {
    ...
    I2cbRegs.I2CSAR.all = 0x25; // Set I2C address to that of DAN777
 ...
 * uint16_t ADC1 = 0;
 * uint16_t ADC2 = 0;
 * err = ReadDAN777ADC(&ADC1,&ADC2);
 * This allows ADC1 and ADC2 to be changed inside the function and return the
...
int16_t ReadDAN777ADC(uint16_t *ADC1, uint16_t *ADC2) {
    ...
    Val1LSB = I2cbRegs.I2CDRR.all; // MMN Read Dan777
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    Val1MSB = I2cbRegs.I2CDRR.all; // MMN Read Dan777
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    Val2LSB = I2cbRegs.I2CDRR.all; // MMN Read Dan777
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    Val2MSB = I2cbRegs.I2CDRR.all; // MMN Read Dan777
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    // Since I2CCNT = 0 at this point, a stop condition will be issued
...
// MMN Write 7 8-bit commands to I2C Slave BQ32000
int16_t WriteBQ32000(uint16_t second,uint16_t minute,uint16_t hour,uint16_t day,uint16_t date,uint16_t month,uint16_t year) {
    // MMN create variables for 8-bit assignment for all 7 registers for BQ32000
    uint16_t secondOnes = 0;
    uint16_t secondTens = 0;
    ...
    int16_t I2C_Xready = 0;
    secondOnes = second%10; // MMN find tens digit of second input
    secondTens = second/10; // MMN find ones digit of second input
    secondWrite = (secondTens << 4) | (secondOnes & 0xF); // MMN bitwise OR tens digit left shifted 4 with ones digit
    minuteOnes = minute%10; // MMN find tens digit of minute input
    minuteTens = minute/10; // MMN find ones digit of second input
    minuteWrite = (minuteTens << 4) | (minuteOnes & 0xF); // MMN bitwise OR tens digit left shifted 4 with ones digit
    hourOnes = hour%10; // MMN find tens digit of hour input
    hourTens = hour/10; // MMN find ones digit of hour input
    hourWrite = (hourTens << 4) | (hourOnes & 0xF); // MMN bitwise OR tens digit left shifted 4 with ones digit
    dayWrite = day & 0x7; // MMN bits 0-2 of command, AND with binary 0111
    dateOnes = date%10; // MMN find tens digit of date input
    dateTens = date/10; // MMN find ones digit of date input
    dateWrite = (dateTens << 4) | (dateOnes & 0xF); // MMN bitwise OR tens digit left shifted 4 with ones digit
    monthOnes = month%10; // MMN find tens digit of month input
    monthTens = month/10; // MMN find ones digit of month input
    monthWrite = (monthTens << 4) | (monthOnes & 0xF); // MMN bitwise OR tens digit left shifted 4 with ones digit
    yearOnes = year%10; // MMN find tens digit of year input
    yearTens = year/10; // MMN find ones digit of year input
    yearWrite = (yearTens << 4) | (yearOnes & 0xF); // MMN bitwise OR tens digit left shifted 4 with ones digit
    // Allow time for I2C to finish up previous commands.
    DELAY_US(200);
    if (I2cbRegs.I2CSTR.bit.BB == 1) { // Check if I2C busy. If it is, it's better
        return 2; // to exit and try again next sample.
    ...
    I2cbRegs.I2CSAR.all = 0x68; // Set I2C address to that of BQ32000
    ...
    I2cbRegs.I2CDXR.all = secondWrite; // MMN Write second command
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Xready = I2C_CheckIfTX(39062); // Poll until I2C ready to transmit
    ...
    I2cbRegs.I2CDXR.all = minuteWrite; // MMN Write minute command
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Xready = I2C_CheckIfTX(39062); // Poll until I2C ready to transmit
    ...
    I2cbRegs.I2CDXR.all = hourWrite; // MMN Write hour command
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Xready = I2C_CheckIfTX(39062); // Poll until I2C ready to transmit
    ...
    I2cbRegs.I2CDXR.all = dayWrite; // MMN Write day command
    // Since I2CCNT = 0 at this point, a stop condition will be issued
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    ...
    I2cbRegs.I2CDXR.all = dateWrite; // MMN Write date command
    // Since I2CCNT = 0 at this point, a stop condition will be issued
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    ...
    I2cbRegs.I2CDXR.all = monthWrite; // MMN Write month command
    // Since I2CCNT = 0 at this point, a stop condition will be issued
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    ...
    I2cbRegs.I2CDXR.all = yearWrite; // MMN Write year command
    // Since I2CCNT = 0 at this point, a stop condition will be issued
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
...
/* Read Two 16 Bit values from I2C Slave BQ32000 starting at BQ32000's register 0.
 * Notice the Rvalue1 and Rvalue2 passed as pointers (passed by reference). So pass
 * address of the uint16_t variable when using this function. For example:
 * uint16_t Rval1 = 0;
 * uint16_t Rval2 = 0;
...
int16_t ReadBQ32000(uint16_t *second,uint16_t *minute,uint16_t *hour,uint16_t *day,uint16_t *date,uint16_t *month,uint16_t *year) {
    ...
    secondRead = I2cbRegs.I2CDRR.all; // MMN Read BQ32000 for seconds 8-bit
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    minuteRead = I2cbRegs.I2CDRR.all; // MMN Read BQ32000 for minutes 8-bit
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    hourRead = I2cbRegs.I2CDRR.all; // MMN Read BQ32000 for hours 8-bit
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    dayRead = I2cbRegs.I2CDRR.all; // MMN Read BQ32000 for days 8-bit
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    dateRead = I2cbRegs.I2CDRR.all; // MMN Read BQ32000 for date 8-bit
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    monthRead = I2cbRegs.I2CDRR.all; // MMN Read BQ32000 for month 8-bit
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    yearRead = I2cbRegs.I2CDRR.all; // MMN Read BQ32000 for year 8-bit
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    // Since I2CCNT = 0 at this point, a stop condition will be issued
    secondOnes = secondRead & 0xF; // MMN bits 0-3 of command, AND with binary 1111
    secondTens = (secondRead >> 4) & 0x7; // MMN bits 4-6 of command, AND with binary 0111
    *second = secondOnes + 10*secondTens; // MMN combine ones and tens digit to get full value
    minuteOnes = minuteRead & 0xF; // MMN bits 0-3 of command, AND with binary 1111
    minuteTens = (minuteRead >> 4) & 0x7; // MMN bits 4-6 of command, AND with binary 0111
    *minute = minuteOnes + 10*minuteTens; // MMN combine ones and tens digit to get full value
    hourOnes = hourRead & 0xF; // MMN bits 0-3 of command, AND with binary 1111
    hourTens = (hourRead >> 4) & 0x3; // MMN bits 4-5 of command, AND with binary 0011
    *hour = hourOnes + 10*hourTens; // MMN combine ones and tens digit to get full value
    days = dayRead & 0x7; // MMN bits 0-2 of command, AND with binary 0111
    *day = days;
    dateOnes = dateRead & 0xF; // MMN bits 0-3 of command, AND with binary 1111
    dateTens = (dateRead >> 4) & 0x3; // MMN bits 4-5 of command, AND with binary 0011
    *date = dateOnes + 10*dateTens; // MMN combine ones and tens digit to get full value
    monthOnes = monthRead & 0xF; // MMN bits 0-3 of command, AND with binary 1111
    monthTens = (monthRead >> 4) & 0x1; // MMN bit 4 of command, AND with binary 0001
    *month = monthOnes + 10*monthTens; // MMN combine ones and tens digit to get full value
    yearOnes = yearRead & 0xF; // MMN bits 0-3 of command, AND with binary 1111
    yearTens = (yearRead >> 4) & 0xF; // MMN bits 4-7 of command, AND with binary 0111
    *year = yearOnes + 10*yearTens; // MMN combine ones and tens digit to get full value
    return 0;
}
...
void main(void)
 {
    ...
    WriteBQ32000(second_actual,minute_actual,hour_actual,day_actual,date_actual,month_actual,year_actual);

    // IDLE loop. Just sit and loop forever (optional):
    while(1)
    {
        if (UARTPrint == 1 ) {
            // MMN write 7 conditions for each day of the week (different value of day_actual)
            if (day_actual == 1 ) {
                serial_printf(&SerialA,"RC1: %d, RC2: %d, ADC1: %.3f, ADC2: %.3f, Sunday %d/%d/%d %d:%d:%d\r\n", RCinp1, RCinp2,
                              ADC1out, ADC2out, month_actual, date_actual, year_actual, hour_actual, minute_actual, second_actual);
            }
            else if (day_actual == 2 ) {
                serial_printf(&SerialA,"RC1: %d, RC2: %d, ADC1: %.3f, ADC2: %.3f, Monday %d/%d/%d %d:%d:%d\r\n", RCinp1, RCinp2,
                              ADC1out, ADC2out, month_actual, date_actual, year_actual, hour_actual, minute_actual, second_actual);
            }
            else if (day_actual == 3 ) {
                serial_printf(&SerialA,"RC1: %d, RC2: %d, ADC1: %.3f, ADC2: %.3f, Tuesday %d/%d/%d %d:%d:%d\r\n", RCinp1, RCinp2,
                              ADC1out, ADC2out, month_actual, date_actual, year_actual, hour_actual, minute_actual, second_actual);
            }
            else if (day_actual == 4 ) {
                serial_printf(&SerialA,"RC1: %d, RC2: %d, ADC1: %.3f, ADC2: %.3f, Wednesday %d/%d/%d %d:%d:%d\r\n", RCinp1, RCinp2,
                              ADC1out, ADC2out, month_actual, date_actual, year_actual, hour_actual, minute_actual, second_actual);
            }
            else if (day_actual == 5 ) {
                serial_printf(&SerialA,"RC1: %d, RC2: %d, ADC1: %.3f, ADC2: %.3f, Thursday %d/%d/%d %d:%d:%d\r\n", RCinp1, RCinp2,
                              ADC1out, ADC2out, month_actual, date_actual, year_actual, hour_actual, minute_actual, second_actual);
            }
            else if (day_actual == 6 ) {
                serial_printf(&SerialA,"RC1: %d, RC2: %d, ADC1: %.3f, ADC2: %.3f, Friday %d/%d/%d %d:%d:%d\r\n", RCinp1, RCinp2,
                              ADC1out, ADC2out, month_actual, date_actual, year_actual, hour_actual, minute_actual, second_actual);
            }
            else if (day_actual == 7 ) {
                serial_printf(&SerialA,"RC1: %d, RC2: %d, ADC1: %.3f, ADC2: %.3f, Saturday %d/%d/%d %d:%d:%d\r\n", RCinp1, RCinp2,
                              ADC1out, ADC2out, month_actual, date_actual, year_actual, hour_actual, minute_actual, second_actual);
            }
            UARTPrint = 0;
        }
        if (RunI2C == 1) {
            RunI2C = 0;
            // Write to DAN777
            I2C_OK = WriteDAN777RCServo(RCinp1, RCinp2);
            ...
            // Read BQ32000
            I2C_OK = ReadBQ32000(&second_actual, &minute_actual, &hour_actual, &day_actual, &date_actual, &month_actual, &year_actual);
...
__interrupt void cpu_timer2_isr(void)
{
    ...
    // MMN print to teraterm every 100 ms
    if ((CpuTimer2.InterruptCount % 5) == 0) {
        UARTPrint = 1;
    }

    // MMN call RunI2C every 20 ms
    if ((CpuTimer2.InterruptCount % 1) == 0) {
        RunI2C = 1;
    }

    // MMN loop to sweep servo direction
    // servo_angle: duty cycle input scaled later from -90 to 90 to 0-100%
    // servo_increment: increment to add to the control input
    ...
    // MMN increment RC servo input values every 20 ms
    RCinp1 += servo_increment;
    RCinp2 += servo_increment;

    // MMN convert ADC value from integer to voltage
    ADC1out = ADC1*3.3/1024;
    ADC2out = ADC2*3.3/1024;
}
