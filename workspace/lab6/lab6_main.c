...
void main(void)
{
    ...
    {
        if (UARTPrint == 1 ) {
            serial_printf(&SerialA,"Left Wheel Angle: %.3f Right Wheel Angle: %.3f Left Wheel Vel: %.3f Right Wheel Vel: %.3f \r\n",leftWheel, rightWheel, vLeft, vRight);
            UARTPrint = 0;
        }
    }
}
...
__interrupt void cpu_timer1_isr(void)
{
    ...
    //VQM Read the wheel angles for left and right
    leftWheel = readEncLeft();
    rightWheel = readEncRight();

    //VQM Convert wheel angle to wheel travel distance
    leftDist = leftWheel/radtoft;
    rightDist = rightWheel/radtoft;

    //VQM Simple discrete derivative based on sample rate for velocity calculation
    vLeft = (leftDist - leftDist_1)/0.004;
    vRight = (rightDist - rightDist_1)/0.004;

    //VQM Calculate error terms for each desired control; turning, moving the left motor and moving the right motor;
    //errorTurn is calculated by determining the difference in velocity between the two wheels to find the current turning rate, and adding the desired turn value;
    //errorLeft and errorRight are calculated by subtracting the current velocity from the reference velocity, and adding the component that we would like to turn as well;
    errorTurn = turn + (vLeft - vRight);
    errorLeft = Vref - vLeft - KpTurn*errorTurn;
    errorRight = Vref - vRight + KpTurn*errorTurn;

    //VQM Integral control, with saturation to prevent integral windup.
    //Uses a simple trapezoidal integration method with previous error values
    ...
    //VQM Setting previous time state values in order for next iteration
    leftDist_1 = leftDist;
    ...
    //VQM These lines compute the current position and bearing of the robot
    //In order to calculate the current x and y position, we need to use the distance traveled by the wheel combined with the current bearing angle
    //The bearing angle is calculated by finding the difference in wheel angle from left to right, and using a geometric relation between the wheel
    //radius and robot width.
    //Then, the x and y velocity by the robot is approximately related to the averaged velocity of the left and right wheels and the bearing angle,
    //which is why we take the cosine and sine of the bearing angle.
    //Then the x and y velocity are also integrated to find the total distance traveled.
    thetaL = leftWheel;
    thetaR = rightWheel;
    ...
    //VQM These are values to receive from LABview, where Vref and turn are commanded by the keyboard
    if (NewLVData == 1) {
        NewLVData = 0;
        Vref = fromLVvalues[0];
        turn = fromLVvalues[1];
    ...
    //VQM These are values to send to LABview, so it can draw the robot position and heading.
