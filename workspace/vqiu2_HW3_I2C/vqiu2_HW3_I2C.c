...
float adc1out = 0.0;
float adc2out = 0.0;
int16_t servo_increment = 10; // VQI RCServo angle increment
uint16_t seconda = 0;
uint16_t minutea = 0;
uint16_t houra = 0;
uint16_t daya = 3;
...
void main(void)
{
    ...
    WriteBQ32000(seconda,minutea,houra,daya,datea,montha,yeara);
    
    // IDLE loop. Just sit and loop forever (optional):
    while(1)
    {
        if (UARTPrint == 1 ) {
            // VQI Need a separate if statement for every single day of the week, do not need to convert to string
            if (daya == 1) {
                serial_printf(&SerialA,"ADC1: %.3f ADC2: %.3f Date: Sunday %d/%d/%d %d:%d:%d\r\n",adc1out, adc2out, montha, datea, yeara, houra, minutea, seconda);
            } else if (daya == 2) {
        ...
        if (RunI2C == 1) {
            ...
            // VQI Read BQ32000 and check for errors
            I2C_OK = ReadBQ32000(&seconda,&minutea,&houra,&daya,&datea,&montha,&yeara);
            while(I2C_OK != 0) {
...
// Write 2 16-bit commands (LSB then MSB) to I2C Slave DAN777 starting at DAN777 register 4
int16_t WriteDAN777RCServo(uint16_t RC1, uint16_t RC2) {
    uint16_t rc1lsb = 0;
    uint16_t rc1msb = 0;
    uint16_t rc2lsb = 0;
    ...
    I2cbRegs.I2CSAR.all = 0x25; // VQI Set I2C address to that of DAN777
...
/* Read Two 16 Bit values from I2C Slave DAN777 starting at DAN777 register 0.
 * Notice the ADC1 and ADC2 passed as pointers (passed by reference). So pass
...
int16_t ReadDAN777ADC(uint16_t *ADC1,uint16_t *ADC2) {
    ...
    I2cbRegs.I2CSAR.all = 0x25; // VQI I2C address of DAN777
...
// Write 7 8-bit commands to I2C Slave BQ32000 starting at BQ32000 register 0
int16_t WriteBQ32000(uint16_t second,uint16_t minute,uint16_t hour,uint16_t day,uint16_t date,uint16_t month,uint16_t year) {
    // To convert from a decimal number to a 4 bit hex value, we need to separate the ones and tens digits
    uint16_t sec_ones = second % 10;
    uint16_t sec_tens = second / 10;
    uint16_t min_ones = minute % 10;
    uint16_t min_tens = minute / 10;
    uint16_t hr_ones = hour % 10;
    uint16_t hr_tens = hour / 10;
    uint16_t date_ones = date % 10;
    uint16_t date_tens = date / 10;
    uint16_t mo_ones = month % 10;
    uint16_t mo_tens = month / 10;
    uint16_t yr_ones = year % 10;
    uint16_t yr_tens = year / 10;
    int16_t I2C_Xready = 0;
    // Add the converted 4 bit hex values together to create the 8 bit value that is sent over I2C
    uint16_t secWrite = ((sec_tens << 4) & 0x70) | (sec_ones & 0xF);
    uint16_t minWrite = ((min_tens << 4) & 0x70) | (min_ones & 0xF);
    uint16_t hrWrite = ((hr_tens << 4) & 0x30)| (hr_ones & 0xF);
    uint16_t dayWrite = day & 0x7;
    uint16_t dateWrite = ((date_tens << 4) & 0x30) | (date_ones & 0xF);
    uint16_t moWrite = ((mo_tens << 4) & 0x10) | (mo_ones & 0xF);
    uint16_t yrWrite = ((yr_tens << 4) & 0xF0) | (yr_ones & 0xF);
    // Allow time for I2C to finish up previous commands.
    DELAY_US(200);
    if (I2cbRegs.I2CSTR.bit.BB == 1) { // Check if I2C busy. If it is, it's better
        return 2; // to exit and try again next sample.
    ...
    I2cbRegs.I2CSAR.all = 0x68; // VQI Set I2C address to that of BQ32000
    I2cbRegs.I2CCNT = 8; // Number of values to send plus start register: 4 + 1
    ...
    I2cbRegs.I2CDXR.all = secWrite; // Write seconds
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Xready = I2C_CheckIfTX(39062); // Poll until I2C ready to transmit
    ...
    I2cbRegs.I2CDXR.all = minWrite; // Write minutes
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Xready = I2C_CheckIfTX(39062); // Poll until I2C ready to transmit
    ...
    I2cbRegs.I2CDXR.all = hrWrite; // Write hours
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Xready = I2C_CheckIfTX(39062); // Poll until I2C ready to transmit
    ...
    I2cbRegs.I2CDXR.all = moWrite; // Write months
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Xready = I2C_CheckIfTX(39062); // Poll until I2C ready to transmit
    ...
    I2cbRegs.I2CDXR.all = yrWrite; // Write years
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    return 0;
...
/* Read 7 16 Bit values from I2C Slave BQ32000 starting at BQ32000 register 0.
 * Notice the ADC1 and ADC2 passed as pointers (passed by reference). So pass
 * address of the uint16_t variable when using this function. For example:
 * uint16_t Rval1 = 0;
 * uint16_t Rval2 = 0;
...
int16_t ReadBQ32000(uint16_t *second,uint16_t *minute,uint16_t *hour,uint16_t *day,uint16_t *date,uint16_t *month,uint16_t *year) {
    ...
    I2cbRegs.I2CSAR.all = 0x68; // VQI I2C address of BQ32000
    I2cbRegs.I2CCNT = 1; // Just sending address to start reading from
    I2cbRegs.I2CDXR.all = 0; // Start reading at this register location
    I2cbRegs.I2CMDR.all = 0x6620; // I2C in master mode (MST), I2C is in transmit mode (TRX) with start
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
    ...
    secRead = I2cbRegs.I2CDRR.all; // Read BQ32000
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    minRead = I2cbRegs.I2CDRR.all; // Read BQ32000
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    hrRead = I2cbRegs.I2CDRR.all; // Read BQ32000
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    *day = I2cbRegs.I2CDRR.all; // Read BQ32000
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    moRead = I2cbRegs.I2CDRR.all; // Read BQ32000
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    I2C_Rready = I2C_CheckIfRX(39062); //Poll until I2C has received 8-bit value
    ...
    yrRead = I2cbRegs.I2CDRR.all; // Read BQ32000
    if (I2cbRegs.I2CSTR.bit.NACK == 1) { // Check for No Acknowledgement
        return 3; // This should not happen
    }
    // Since I2CCNT = 0 at this point, a stop condition will be issued
    // We received the 8 bit value from I2C but we need to convert it back to a decimal
    *second = ((secRead >> 4) & 0x7)*10 + (secRead & 0xF);
    *minute = ((minRead >> 4) & 0x7)*10 + (minRead & 0xF);
    *hour = ((hrRead >> 4) & 0x3)*10 + (hrRead & 0xF);
    *date = ((dateRead >> 4) & 0x3)*10 + (dateRead & 0xF);
    *month = ((moRead >> 4) & 0x1)*10 + (moRead & 0xF);
    *year = ((yrRead >> 4) & 0xF)*10 + (yrRead & 0xF);
    return 0;
}
